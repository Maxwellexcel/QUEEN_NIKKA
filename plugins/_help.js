...
👉𝙳𝙰𝚃𝙴 - ${message.date} 
👉𝙲𝚄𝚁𝚁𝙴𝙽𝚃 𝚃𝙸𝙼𝙴 - ${message.time} 
👉𝙲𝚁𝙴𝙰𝚃𝙾𝚁- *Star King*
👉𝚄𝚂𝙴𝚁 - ${Config.ownername} 
➮Nᴜᴍ - ${owner.split(",")[0]} 
  ...
  async (message, query, { Void }) => {
    try {
      ...
      if (!isSticker && queryParts.length > 1) {
        ...
        return await message.send(
          "*_Huh please, provide Cmd With New Name_*\n*Eg: _.setcmd New_Name, Cmd_Name_*"
        );
      }

      if (newCommand.length < 1) {
        return await message.reply(
          "*_Huh Please, Provide New_Cmd Name First_*"
        );
      }
      ...
      if (foundCommand) {
        ...
        return await message.send(
          `*_Cmd "${global.setCmdAlias[newCommand]}" Has been Successfully set to "${
  ...
  async (message, query, { Void }) => {
    try {
      ...
      if (message.quoted) {
        ...
        return await message.send(
          "*_huh please, provide the name that was set for a command_*\n*Eg: _.delcmd Cmd_Name_*"
        );
      }
...
astro_patch.cmd(
  ...
  async (message, query) => {
    try {
      ...
      if (!textToTranslate) {
        return await message.reply(
          `*Kindly provide the text to translate. Example: ${prefix}trt en Who are you*`
        );
      }
